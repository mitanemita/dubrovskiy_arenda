...
4. Запустить (миграции применятся автоматически перед стартом):
   ...
   ```
5. Первичная инициализация (арендодатель + владельцы + настройки) выполняется
   **автоматически** при старте бота (идемпотентно). Отдельно запускать не нужно.
   При необходимости её можно выполнить вручную:
   ```bash
   docker compose run --rm api python -m app.scripts.bootstrap
   ```
   Чтобы у оператора был доступ и не было ошибки «Нет арендодателя», укажите свой
   Telegram ID в `ADMIN_IDS` в `.env` **до** первого запуска.
6. Проверка:
...
7. Заполнение данных через бота (раздел **🗂 Справочники** в меню):
   - **🏦 Реквизиты арендодателя** — наименование, ИНН, КПП, адрес, банк/БИК/счета
     (нужны для УПД и квитанций); заполните до генерации документов.
   - **Помещение** (у каждого статус 🟢 свободно / 🔴 занято, меняется по кнопке),
     затем **Арендатор** (для УПД укажите КПП и адрес), потом **Договор**
     (выбираете арендатора и помещение), при необходимости — **Счётчик**.
   После этого работают «Показания», «Отметить оплату», «Отчёты» и вебхуки n8n
   (сопоставление платежей/показаний по договору/ИНН/счётчику).

> Меню бота — на инлайн-кнопках под сообщением. Если после обновления снизу
> осталась старая клавиатура — нажмите `/start`, она уберётся автоматически.

> Для проверки функционала есть команда `/admin` (только операторам):
> «🧪 Заполнить тестовыми данными» — создаёт демо-набор (помещения, арендаторы,
> договоры, счётчик, задачи) и заполняет реквизиты-заглушки; «🗑 Очистить все
> данные» — удаляет все бизнес-данные (арендодатель и настройки сохраняются).
...
```bash
cd dubrovskiy_arenda
git pull
docker compose up -d --build   # пересборка + сервис migrate применит новые миграции
docker compose logs -f bot     # при желании — смотреть логи бота (выход: Ctrl+C)
```
Данные в PostgreSQL сохраняются (том `pg_data`), миграции применяются автоматически.

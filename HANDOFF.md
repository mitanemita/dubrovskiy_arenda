...
- Уведомления по задачам больше НЕ шлём в чат: `dispatch_telegram` пропускает
  тип `task_reminder` (см. `get_pending(exclude_types=...)`), а сами уведомления
  копятся как «входящие» (status=queued). Счётчик непрочитанных
  (`task_service.count_task_notifications`) показывается жирной плашкой вверху всех
  меню (кроме раздела «Задачи», где он в заголовке и в строке «🔔 Уведомления (N)»).
  Раздел `tasks:notif` — список уведомлений с кнопками-номерами; `npick:<id>`
  открывает карточку задачи и помечает уведомление прочитанным; «Прочитать все» и
  выполнение/удаление задачи тоже гасят её уведомления
  (`mark_notification_read` / `mark_task_notifications_read` / `mark_all_...`).
- Смена категории задачи пересчитывает срок от СЕГОДНЯ (`update_task(today=...)`),
  а не от даты создания (иначе срок уезжал в прошлое).

...
- Кнопки напоминания получили отдельный префикс `rt…` (rtdone/rtcat/rtdate):
  после действия сообщение-напоминание удаляется, чтобы не копить их в чате
  (`_delete_message_safe`, флаг `remind_msg` в FSM). Карточка задачи по-прежнему
  на `taskdone/taskcat/taskdate` и работает как раньше.

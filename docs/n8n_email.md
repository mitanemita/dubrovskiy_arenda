...
| UniOne **403, code 229 «Custom backend domain or tracking domain required»** | Нужен домен трекинга (или отключённый трекинг). Быстро: в теле запроса передавайте `track_links: 0, track_read: 0` (уже в воркфлоу). Если требование остаётся — в UniOne **Настройки → Домены → Домен для трекинга** добавьте поддомен (напр. `track.ваш-домен.ru`), пропишите CNAME из панели в DNS и дождитесь подтверждения. И подтвердите сам домен отправки (DKIM/SPF). |

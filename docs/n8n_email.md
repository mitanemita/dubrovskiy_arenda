...
| UniOne **403, code 229 «Custom backend domain or tracking domain required»** | Требование **уровня аккаунта** — флаги `track_links/track_read=0` его НЕ снимают. Нужно в UniOne настроить **домен для трекинга**: **Настройки → Домены → Домен для трекинга (Tracking domain)** → добавьте поддомен (напр. `track.ваш-домен.ru`), пропишите показанный **CNAME** в DNS и дождитесь статуса «подтверждён». Плюс подтвердите **сам домен отправки** (DKIM/SPF), с которого шлёте `from_email`. После подтверждения обоих отправка разблокируется. |
